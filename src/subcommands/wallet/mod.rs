...
use crate::utils::{
    ...
    other::{
        check_capacity, get_arg_value, get_address, get_live_cell_with_cache, get_master_key_signer_raw,
        ...
        serialize_signature_bytes, sync_to_tip
    },
...
impl<'a> WalletSubCommand<'a> {
    ...
    //        helper.sign_inputs(signer, &mut get_live_cell_fn, is_ledger, change_path, skip_check)?
    //    {
    ...
    pub fn transfer(
        ...
        for (lock_arg, signature) in
            helper.sign_inputs(signer, &mut get_live_cell_fn, is_ledger, skip_check)?
        {
